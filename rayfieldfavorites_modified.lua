	...
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				...
				RayfieldLibrary:Notify({Title = "CatBypasser", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end
...
-- Add "Favorite" button overlay to main UI buttons
local function createFavoriteButton(button)
    local favoriteButton = Instance.new("ImageButton")
    favoriteButton.Size = UDim2.new(0, 20, 0, 20)  -- Small overlay size
    favoriteButton.Position = UDim2.new(1, -25, 0, 5)  -- Positioned at top-right of main button
    favoriteButton.Image = "rbxassetid://<your_image_id>"  -- Replace with actual image ID for favorite icon
    favoriteButton.BackgroundTransparency = 1
    favoriteButton.Parent = button

    -- Functionality when clicking the favorite button
    favoriteButton.MouseButton1Click:Connect(function()
        -- Example of toggling favorite state (you can customize)
        button:SetAttribute("Favorited", not button:GetAttribute("Favorited"))
        print("Favorite button clicked. Favorited:", button:GetAttribute("Favorited"))
    end)
end

-- Assuming 'mainButton' represents existing buttons you want to overlay the favorite button on
-- Example usage with a loop if there are multiple buttons
for _, button in pairs(script.Parent:GetChildren()) do
    if button:IsA("TextButton") or button:IsA("ImageButton") then
        createFavoriteButton(button)
    end
end
